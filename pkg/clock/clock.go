...
func (tl TimeInterval) WaitNext() <-chan time.Time {
	now := time.Now()
	if now.After(tl.Start) && now.Before(tl.End) {
...
func NewTimeInterval(intervalConf string) (TimeInterval, error) {
	...
	if len(clocks) != 2 {
		return TimeInterval{}, fmt.Errorf("invalid notify time %s (format HH:MM-HH:MM)", intervalConf)
	}
	t1, err := time.Parse("15:04", clocks[0])
	if err != nil {
		return TimeInterval{}, fmt.Errorf("invalid clock %s (format HH:MM)", clocks[0])
	}
	t2, err := time.Parse("15:04", clocks[1])
	if err != nil {
		return TimeInterval{}, fmt.Errorf("invalid clock %s (format HH:MM)", clocks[1])
	}

	if t2.Before(t1) {
		return TimeInterval{}, fmt.Errorf("invalid interval %s", intervalConf)
	}
